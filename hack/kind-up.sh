...
setup_kind_with_lpp_resize_support() {
  ...
  curl "https://raw.githubusercontent.com/marjus45/local-path-provisioner/feature-external-resizer/deploy/local-path-storage.yaml" | \
    sed 's|image: rancher/local-path-provisioner:master-head|image: ismailalidzhikov694/local-path-provisioner:feature-external-resizer-c0c1c13|g' | \
    kubectl apply -f -
