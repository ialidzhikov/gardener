...
var _ = ginkgo.Describe("Shoot worker operation testing", func() {
	...
	f.Default().Serial().CIt("should add one machine to the worker pool and remove it again", func(ctx context.Context) {
		...
		var (
			minimum = shoot.Spec.Provider.Workers[0].Minimum + 1
			maximum = shoot.Spec.Provider.Workers[0].Maximum
		)
		if shoot.Spec.Provider.Workers[0].Maximum < minimum {
			maximum = minimum
		}

		ginkgo.By(fmt.Sprintf("updating shoot worker to min of %d machines", minimum))
		err := f.UpdateShoot(ctx, func(shoot *gardencorev1beta1.Shoot) error {
			shoot.Spec.Provider.Workers[0].Minimum = minimum
			shoot.Spec.Provider.Workers[0].Maximum = maximum
			return nil
		})
		...
		minimum = shoot.Spec.Provider.Workers[0].Minimum - 1
		maximum = shoot.Spec.Provider.Workers[0].Maximum - 1
