...
func GetKubeObjectsAsYamlBytes(deploymentName, namespace, containerImageName, serverSecretName string) (map[string][]byte, error) {
	...
	return registry.AddAllAndSerialize(
		...
		makeEndpointEditorClusterRole(),
		makeEndpointEditorClusterRoleBinding(namespace),
		makePodReaderClusterRole(),
		makePodReaderClusterRoleBinding(namespace),
		makeSecretReaderClusterRole(),
		makeSecretReaderClusterRoleBinding(namespace),
		makeLeaseEditorRole(namespace),
		makeLeaseEditorRoleBinding(namespace),
