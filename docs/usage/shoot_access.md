...
> Note: For Shoot clusters using Kubernetes version >= 1.26 the `enableStaticTokenKubeconfig` field is locked to `false` - cannot be set to `true`. The [`shoots/adminkubeconfig` subresource](#shootsadminkubeconfig-subresource) should be used instead.
